...
-   the `add_one` UDF produces a `pixeltable.udf.add_one` span under
    each `pixeltable.row`
...
-   **Traces**: Explore -\> your Traces (Tempo) data source -\> search
    ...
    `pixeltable.store.build_rows`/`pixeltable.sa.insert_rows` write
    path, and the `pixeltable.view_load` subtree with its own rows and
    store writes.
