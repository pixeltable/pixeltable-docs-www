...
first table operation. It configures itself from the database’s
`[[pixeltable.database]]` entry in `pixeltable.toml`, from environment
variables, or from the `[otel]` section of `~/.pixeltable/config.toml`,
in that order of precedence.

**Option 1: the database entry in `pixeltable.toml`.** Put the endpoint
on the `[[pixeltable.database]]` entry of the database (the entry with
no `name` is the local database; see
[Configuration](../platform/configuration#per-database-settings)). The
headers are a secret and stay out of the file: keep
`OTEL_EXPORTER_OTLP_HEADERS` in the environment, and on a hosted
database set it with
`pxt secret set pxt://org:db OTEL_EXPORTER_OTLP_HEADERS=...`, then
`pxt db restart` so the running database sees it.


```toml
[[pixeltable.database]]
db_exporter_otlp_endpoint = 'https://otlp-gateway-<zone>.grafana.net/otlp'
```

**Option 2: environment variables.** Set these **before starting the
...
**Option 3: `~/.pixeltable/config.toml`**, shared by every database on
the installation:
