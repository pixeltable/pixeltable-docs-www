...
## title: 'Computed Columns' description: 'Computed columns combines automatic calculations with smart incremental updates. Think of them as your data workflow automated.' icon: 'blank' mode: 'wide'

<Info>
Learn more about computed columns with our [in-depth guide](/tutorials/computed-columns).
</Info>
...
pop_t = pxt.io.import_csv(
    ...
    'https://github.com/pixeltable/pixeltable/raw/main/docs/resources/world-population-data.csv'
)
...
url_prefix = 'https://github.com/pixeltable/pixeltable/raw/main/docs/resources/images'
...
```python
# First computed column: calculate total population change over 23 years
pop_t.add_computed_column(total_change=pop_t.pop_2023 - pop_t.pop_2000)

# Second computed column: calculate average yearly change using the first column
pop_t.add_computed_column(
    avg_yearly_change=pop_t.total_change / 23
)
```
...
<CardGroup cols={2}>
  ...
  <Card title="UDF Examples" icon="code" href="/platform/custom-functions">
