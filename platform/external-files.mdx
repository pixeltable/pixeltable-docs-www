...
- `pxt.Image` (Python value: `PIL.Image.Image`)
- `pxt.Video` (Python value: a local path string)
- `pxt.Audio` (Python value: a local path string)
...
Media columns on a `TableModel` are the same types as in this notebook:
...
```bash
pxt schema update app.py external_data
```
