...
## title: "Production Deployment" description: "Guide to deploying Pixeltable in production environments"
...
- **[Multimodal Types](/platform/bringing-data):** Native handling of Video, Document, Audio, Image, JSON.
- **[Computed Columns](/platform/computed-columns):** Automatic incremental updates and dependency tracking.
...
<Note>
**Key Principles:**
- **Module UDFs** (`functions.py`): Update when code changes; improve testability. [Learn more](/platform/custom-functions)
- **Retrieval Queries** (`@pxt.query`): Encapsulate complex retrieval logic as reusable functions.
- **Idempotency:** Use `if_exists='ignore'` to make `setup_pixeltable.py` safely re-runnable. [Learn more](/platform/tables)
</Note>
...
- **Embedding Indexes:** pgvector for efficient similarity search. [Learn more](/platform/vector-database)
