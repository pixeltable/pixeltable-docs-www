...
## title: 'Bringing Data' description: 'A comprehensive guide to inserting, referencing, and importing data in Pixeltable' icon: 'blank'
...
# Insert array data (pxt.Float columns require float32 dtype)
array_table.insert([{
    'fixed_vector': np.random.randn(768).astype(np.float32),
    'variable_tensor': np.random.randn(5, 512).astype(np.float32),
    'any_int_array': np.array([1, 2, 3, 4]),
    'any_float_array': np.random.randn(3, 3).astype(np.float32),
    'completely_flexible': np.array([[1, 2], [3, 4]])
}])
```
````
