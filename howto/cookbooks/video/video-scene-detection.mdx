...
**What’s in this recipe:** - Detect hard cuts with
`scene_detect_content()` - Find fade transitions with
`scene_detect_threshold()` - Use adaptive detection with
...
print(f"Loaded {videos.count()} video(s)")
...
result = videos.select(videos.title, videos.scenes_content).collect()
for row in result:
    print(f"\n{row['title']}:")
    for i, scene in enumerate(row['scenes_content']):
        print(f"  Scene {i+1}: {scene['start_time']:.2f}s - duration {scene['duration']:.2f}s")
...
result = videos.select(videos.title, videos.scenes_fade).collect()
for row in result:
    print(f"\n{row['title']} (fade detection):")
    for i, scene in enumerate(row['scenes_fade']):
        print(f"  Scene {i+1}: {scene['start_time']:.2f}s - duration {scene['duration']:.2f}s")
...
result = videos.select(videos.title, videos.scenes_adaptive).collect()
for row in result:
    print(f"\n{row['title']} (adaptive detection):")
    for i, scene in enumerate(row['scenes_adaptive']):
        print(f"  Scene {i+1}: {scene['start_time']:.2f}s - duration {scene['duration']:.2f}s")
