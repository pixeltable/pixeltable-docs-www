---
title: "Join tables to combine data"
sidebarTitle: "Join tables to combine data"
icon: "notebook"
---
...
**What’s in this recipe:** - Inner join to match rows from both tables -
Left join to keep all rows from the first table - Cross join for
Cartesian product (all combinations) - Join with filtering, aggregation,
and saving results
...
&lt;pixeltable.catalog.dir.Dir&nbsp;at&nbsp;0x3132865d0&gt;
</pre>
...
Inserting&nbsp;rows&nbsp;into&nbsp;\`customers\`:&nbsp;3&nbsp;rows&nbsp;\[00:00,&nbsp;270.48&nbsp;rows/s\]
...
Inserting&nbsp;rows&nbsp;into&nbsp;\`orders\`:&nbsp;4&nbsp;rows&nbsp;\[00:00,&nbsp;2076.90&nbsp;rows/s\]
...
Inserting&nbsp;rows&nbsp;into&nbsp;\`products\`:&nbsp;2&nbsp;rows&nbsp;\[00:00,&nbsp;1012.14&nbsp;rows/s\]
...
Inserting&nbsp;rows&nbsp;into&nbsp;\`orders_report\`:&nbsp;3&nbsp;rows&nbsp;\[00:00,&nbsp;1017.62&nbsp;rows/s\]
Inserted&nbsp;3&nbsp;rows&nbsp;with&nbsp;0&nbsp;errors.
</pre>
...
**Tips:** - Use explicit predicates (`t1.col == t2.col`) for clarity -
Chain `.where()` after join to filter results - Chain `.group_by()` for
aggregations - Use `'left'` join when the first table is your “main”
table - Use named columns in `.select(name=col)` for clean column names
