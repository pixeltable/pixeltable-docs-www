...
- Declare HTTP services and see what serving them would change, with
  `pxt service`
...
One file declares the models that name the tables and the services that
serve routes over them. `pxt service example` writes one to start from;
`pxt service diff` reports what serving it would change, without
starting anything. See [HTTP
