---
title: "Create custom aggregate functions (UDAs)"
sidebarTitle: "Create custom aggregate functions (UDAs)"
icon: "notebook"
---
...
**What’s in this recipe:** - Define a UDA (User-Defined Aggregate) with
the `@pxt.uda` decorator - Use UDAs in `group_by` queries - Create UDAs
...
&lt;pixeltable.catalog.dir.Dir&nbsp;at&nbsp;0x16bfa64d0&gt;
</pre>
...
<pre style={{ 'margin': '-20px 20px 0px 20px', 'padding': '0px', 'background-color': 'transparent', 'color': 'black' }}>
Created&nbsp;table&nbsp;&#x27;sales&#x27;.
Inserting&nbsp;rows&nbsp;into&nbsp;\`sales\`:&nbsp;6&nbsp;rows&nbsp;\[00:00,&nbsp;415.71&nbsp;rows/s\]
...
**Key points:** - Always handle `None` values in `update()` - Multiple
parameters in `update()` enable multi-column aggregations (like
`weighted_avg`) - Return type annotation on `value()` determines output
column type
