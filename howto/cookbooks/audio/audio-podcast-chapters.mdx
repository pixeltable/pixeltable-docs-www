...
-   One LLM call per segment instead of one batched call — fine for
    short podcasts, but consider batching for episodes with 50+ segments
-   This approach requires running a full transcription model just to
    find where the pauses are
