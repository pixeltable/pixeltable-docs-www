...
**What’s in this recipe:** - Generate speech with OpenAI TTS - Choose
from multiple voice options - Store text and audio together
...
**Tips:** - Use `tts-1` for drafts and real-time applications - Use
`tts-1-hd` for final production audio - Audio is cached—no regeneration
on queries
