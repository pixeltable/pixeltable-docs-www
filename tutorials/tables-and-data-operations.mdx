...
```python
eq_t.add_column(note=(pxt.String | None))
```
...
Here, `note` is the column name, and `pxt.String | None` specifies the
type of the new column. Note that the new column *must* be nullable
(`| None`) so that Pixeltable can fill in empty values for existing
rows.


```python
eq_t.add_column(contact_email=(pxt.String | None))
```
...
eq_t.add_column(map_image=(pxt.Image | None))
eq_t.describe()
```
...
This notebook used `create_table` so you can run cells without a
project. Columns you added later with `add_column` belong on the class
only after you write those fields. Starting schema in `app.py`:
...
class Films(TableModel, name='films'):
...
```bash
pxt schema update app.py fundamentals
```
