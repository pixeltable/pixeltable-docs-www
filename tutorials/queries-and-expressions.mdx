...
creates the tables with `pxt schema update`. Once the table exists,
`select()` is the same in an app.
...
Once the table exists, `select()` is the same in an app. To create this
table from a file, put the columns on a `TableModel` (CSV/HF import
stays `create_table(..., source=)` in notebooks):
...
class Mnist(TableModel, name='mnist'):
    image: pxt.Image
    label: pxt.Int
```
