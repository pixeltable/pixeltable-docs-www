...
```python
detr_for_object_detection()-> Json
```
...
```python
sentence_transformer()-> Array[(None,), Float]
```
...
```python
sentence_transformer_list()-> Json
```
...
```python
speech2text_for_conditional_generation()-> String
```
...
```python
vit_for_image_classification()-> Json
```
