...
```python
delete(self, where: exprs.Expr | None = None) -> UpdateStatus
```
...
```python
get_metadata(self) -> TableMetadata
```
...
```python
get_versions(self, n: int | None = None) -> list[VersionMetadata]
```
...
```python
history(self, n: int | None = None) -> pd.DataFrame
```
...
```python
list_views(self, *, recursive: bool = True) -> list[str]
```
...
```python
rename_column(self, old_name: str, new_name: str) -> None
```
