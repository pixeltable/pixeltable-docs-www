...
```python
collect(self) -> DataFrameResultSet
```
...
```python
group_by(self, *grouping_items: Any) -> DataFrame
```
...
```python
head(self, n: int = 10) -> DataFrameResultSet
```
...
```python
select(self, *items: Any, **named_items: Any) -> DataFrame
```
...
```python
show(self, n: int = 20) -> DataFrameResultSet
```
...
```python
tail(self, n: int = 10) -> DataFrameResultSet
```
...
```python
where(self, pred: exprs.Expr) -> DataFrame
```
