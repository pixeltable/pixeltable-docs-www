...
```python Signature
@pxt.udf
yolo_to_coco(detections: pxt.Json) -> pxt.Json[(Json[{'bbox': Json[(Int, ...)], 'category': Int}], ...)]
```
...
- `pxt.Json[(Json[{'bbox': Json[(Int, ...)], 'category': Int}], ...)]`: A dictionary containing the data from `detections`, converted to COCO format.
...
- `YoloxResponse`: A dictionary containing the output of the object detection model.
