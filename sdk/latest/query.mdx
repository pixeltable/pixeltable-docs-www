...
title: "Query"
icon: "square-c"
description: "<a href=\"https://github.com/pixeltable/pixeltable/blob/main/pixeltable/_query.py#L368\" id=\"viewSource\" target=\"_blank\" rel=\"noopener noreferrer\"><img src=\"https://img.shields.io/badge/View%20Source%20on%20Github-blue?logo=github&labelColor=gray\" alt=\"View Source on GitHub\" style={{ display: 'inline', margin: '0px' }} noZoom /></a>"
---
...
```python Signatures
# Signature 1:
group_by(grouping_tbl: catalog.Table, /) -> Self

# Signature 2:
group_by(*grouping_items: exprs.Expr) -> Self
```
...
- **`grouping_items`** (`exprs.Expr | catalog.Table`): expressions to group by

**Returns:**

- `Self`: A new query with the specified group-by clause.
...
- **`how`** (`JoinType.LiteralType`, default: `'inner'`): the type of join to perform.
...
```python Signature
limit(n: int, offset: int | None = None) -> Self
```
...
- `Self`: A new query with the specified limited rows.
...
```python Signature
order_by(*expr_list: exprs.Expr, asc: bool = True) -> Self
```
...
- `Self`: A new query with the specified order-by clause.
...
- `Self`: A new query which specifies the sampled rows
...
```python Signature
select(*items: Any, **named_items: Any) -> Self
```
...
- `Self`: A new query with the specified select list.
...
- `Self`: A new query with the specified predicates replacing the where-clause.
