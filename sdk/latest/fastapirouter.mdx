...
title: "FastAPIRouter"
icon: "square-c"
description: "<a href=\"https://github.com/pixeltable/pixeltable/blob/main/pixeltable/serving/_fastapi.py#L400\" id=\"viewSource\" target=\"_blank\" rel=\"noopener noreferrer\"><img src=\"https://img.shields.io/badge/View%20Source%20on%20Github-blue?logo=github&labelColor=gray\" alt=\"View Source on GitHub\" style={{ display: 'inline', margin: '0px' }} noZoom /></a>"
---
...
add_delete_route(
    ...
    *,
    path: str,
    match_columns: ColumnsParam | None = None,
...
- **`t`** (`RouteTarget`): The table to delete from, or the model that declares it.
- **`path`** (`str`): The URL path for the endpoint.
- **`match_columns`** (`ColumnsParam | None`): Columns to match on (AND-ed equality). Defaults to the table's primary key.
    Must be non-empty.
...
- **`t`** (`RouteTarget`): The table to insert into, or the model that declares it.
- **`path`** (`str`): The URL path for the endpoint.
- **`inputs`** (`ColumnsParam | None`): Columns to accept as request fields. Defaults to all non-computed columns.
- **`uploadfile_inputs`** (`ColumnsParam | None`): Columns to accept as
...
- **`outputs`** (`ColumnsParam | None`): Columns to include in the response. Defaults to all columns (including inputs).
...
- **`inputs`** (`ColumnsParam | None`): Parameters to accept as request fields. Defaults to all query parameters.
- **`uploadfile_inputs`** (`ColumnsParam | None`): Parameters to accept as
    [`UploadFile`](https://fastapi.tiangolo.com/tutorial/request-files/) fields
    (must be media-typed).
...
- **`t`** (`RouteTarget`): The table to update, or the model that declares it.
- **`path`** (`str`): The URL path for the endpoint.
- **`inputs`** (`ColumnsParam | None`): Columns to accept as request fields, excluding primary key and media-typed
    columns (which cannot be updated). Defaults to all non-computed, non-primary-key,
    non-media columns.
- **`outputs`** (`ColumnsParam | None`): Columns to include in the response. Defaults to all columns (including
    inputs).
- **`return_fileresponse`** (`bool`, default: `False`): If True, return the single media-typed output column as a
    [`FileResponse`](https://fastapi.tiangolo.com/advanced/custom-response/#fileresponse).
    Requires exactly one media-typed output column.
...
- **`uploadfile_inputs`** (`ColumnsParam | None`): Columns to accept as
    ...
    (must be media-typed). These are sent as multipart form data; all other inputs
    become [`Form`](https://fastapi.tiangolo.com/tutorial/request-forms/) fields.
- **`outputs`** (`ColumnsParam | None`): Columns from the inserted row to pass to the decorated function as keyword
...
- **`t`** (`RouteTarget`): The table to update, or the model that declares it.
- **`path`** (`str`): The URL path for the endpoint.
- **`inputs`** (`ColumnsParam | None`): Columns to accept as update fields. Defaults to all non-computed, non-primary-key,
    non-media columns.
- **`outputs`** (`ColumnsParam | None`): Columns from the updated row to pass to the decorated function as keyword
    arguments. Defaults to all columns.
- **`export_sql`** (`SqlExport | None`): If set, export the decorated function's return value into an external
