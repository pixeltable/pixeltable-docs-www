...
icon: "square-m"
description: "Pixeltable UDFs that wrap OpenAI chat, embedding, image, Whisper, and tool-calling endpoints as computed columns inside Pixeltable tables."
---
...
- `ImagesResponse`: A dictionary containing the edited image data. Images will be deserialized into `PIL.Image.Image` objects,
...
- `ImagesResponse`: A dictionary containing the generated image data. Images will be deserialized into `PIL.Image.Image` objects,
  and the result dictionary will have the following form:
  ```json
  {
      "created": 1234567890,
...
- `ImagesResponse`: A dictionary containing the variation image data. Images will be deserialized into `PIL.Image.Image` objects,
  and the result dictionary will have the following form:
  ```json
  {
      "created": 1234567890,
...
- `ModerationCreateResponse`: Details of the moderations results.
...
## <span style={{ 'color': 'gray' }}>udf</span>&nbsp; responses()
...
- `TranscriptionResponse`: A dictionary containing the transcription and other metadata.
...
- `TranslationResponse`: A dictionary containing the translation and other metadata.
