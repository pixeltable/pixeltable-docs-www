...
title: "Table"
icon: "square-c"
description: "<a href=\"https://github.com/pixeltable/pixeltable/blob/main/pixeltable/catalog/table.py#L34\" id=\"viewSource\" target=\"_blank\" rel=\"noopener noreferrer\"><img src=\"https://img.shields.io/badge/View%20Source%20on%20Github-blue?logo=github&labelColor=gray\" alt=\"View Source on GitHub\" style={{ display: 'inline', margin: '0px' }} noZoom /></a>"
noindex: true
---
...
## <span style={{ 'color': 'gray' }}>method</span>&nbsp; add_btree_index()
...
- **`column`** (`str | ColumnRef`): The name of, or reference to, the column to be indexed; must be an indexable scalar or media
    column (see above).
- **`idx_name`** (`str | None`): An optional name for the index. If not specified, a unique name will be generated automatically.
    If specified, the name must be unique for this table and a valid pixeltable column name.
- **`if_exists`** (`Literal['error', 'ignore']`, default: `'error'`): Directive for handling an existing B-tree index on the same column, or an existing index with
...
- **`kwargs`** (`TypeForm | ColumnSpec`): Exactly one keyword argument of the form `col_name=type` or `col_name=col_spec_dict`,
...
- **`schema`** (`Mapping[str, TypeForm | ColumnSpec]`): A dictionary mapping column names to a `type` or a [`ColumnSpec`](./columnspec) dict.
...
- **`column`** (`str | ColumnRef`): The name of, or reference to, the column to be indexed; must be a `String`, `Image`, `Audio`,
...
- **`embedding`** (`Function | None`): The UDF to use for the embedding. Must be a UDF that accepts a single argument of type `String`
    ...
    supplied.
- **`string_embed`** (`Function | None`): An optional UDF to use for the string embedding component of this index.
- **`image_embed`** (`Function | None`): An optional UDF to use for the image embedding component of this index.
- **`audio_embed`** (`Function | None`): An optional UDF to use for the audio embedding component of this index.
- **`video_embed`** (`Function | None`): An optional UDF to use for the video embedding component of this index.
- **`document_embed`** (`Function | None`): An optional UDF to use for the document embedding component of this index.
...
- **`if_exists`** (`Literal['error', 'ignore', 'replace', 'replace_force']`, default: `'error'`): Directive for handling an existing index. The existing index is the one with the same name
...
## <span style={{ 'color': 'gray' }}>method</span>&nbsp; alter_computed_column()
...
Args:
    ...
    recompute: If `True`, the stored values of the column are recomputed. If `False`, they remain those
        ...
        [`recompute_columns()`](./table#method-recompute_columns).
...
Returns:
    An [`UpdateStatus`](./updatestatus) describing the recompute; rows that raise during
...
## <span style={{ 'color': 'gray' }}>method</span>&nbsp; compute()
...
- **`source`** (`Sequence[dict[str, Any]] | Sequence[pydantic.BaseModel]`): Rows to compute, as a sequence of dictionaries or Pydantic model instances. Rows contain
    ...
    [`insert()`](./table#method-insert) apply.
- **`on_error`** (`Literal['abort', 'ignore']`, default: `'abort'`): Determines the behavior if an error occurs while evaluating a computed column or detecting an
    ...
    - If `on_error='ignore'`, execution will continue and the (possibly partially) completed rows will be
        ...
        error recorded in the row's [`errors`](./row). A row whose view filter fails to
        evaluate is dropped.

**Returns:**

- `RowBatch`: A [`RowBatch`](./rowbatch) of output rows, in input row order (with an iterator's output
  rows in iteration order). Each [`Row`](./row) contains a value for every column of the
  table. [`Row.errors`](./row) holds `{'errortype': ..., 'errormsg': ...}` for each cell that raised,
...
## <span style={{ 'color': 'gray' }}>method</span>&nbsp; cursor()
...
```python Signatures
# Signature 1:
group_by(grouping_tbl: Table, /) -> Query

# Signature 2:
group_by(*grouping_items: exprs.Expr) -> Query
```
...
- `'Query'`: A Query with the specified limited rows.
...
```python Signature
order_by(*items: exprs.Expr, asc: bool = True) -> Query
```
