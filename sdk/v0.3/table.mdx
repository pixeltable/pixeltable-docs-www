...
<a href="https://github.com/pixeltable/pixeltable/blob/v0.3/pixeltable/catalog/table.py#L48" target="_blank">View source on GitHub</a>
...
- **`kwargs`** (*Union[ts.ColumnType, builtins.type, _GenericAlias, exprs.Expr]*): Exactly one keyword argument of the form `col_name=col_type`.
...
- **`column`** (*Union[str, ColumnRef]*): The name of, or reference to, the column to be indexed; must be a `String` or `Image` column.
...
- **`column`** (*Union[str, ColumnRef]*): The name or reference of the column to drop.
...
- **`column`** (*Union[str, ColumnRef, None]*): The name of, or reference to, the column from which to drop the index. The column must have only one embedding index.
...
### `base_table` <sub>property</sub>

**Type:** property (read-only)

---

### `columns` <sub>property</sub>
...
### `external_stores` <sub>property</sub>
