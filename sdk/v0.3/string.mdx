...
<a href="https://github.com/pixeltable/pixeltable/blob/v0.3/pixeltable/functions/string.py#L0" target="_blank">View source on GitHub</a>
...
Equivalent to [`str.replace()`](https://docs.python.org/3/library/stdtypes.html#str.replace) or [`re.sub()`](https://docs.python.org/3/library/re.html#re.sub), depending on the value of regex.
...
- **`n`** (*Int*): number of replacements to make (-1 for all)
- **`case`** (*Bool*): if False, ignore case
- **`flags`** (*Int*): [flags](https://docs.python.org/3/library/re.html#flags) for the `re` module
- **`regex`** (*Bool*): if True, treat pattern as a regular expression
