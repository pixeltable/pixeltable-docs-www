...
icon: "square-m"
---
<a href="https://github.com/pixeltable/pixeltable/blob/v0.3/pixeltable/__init__.py#L0" target="_blank">View source on GitHub</a>
...
create_replica(
    destination: str,
    source: Union[str, catalog.Table]
)-> Optional[catalog.Table]
```

**Parameters:**

- **`destination`** (*str*): Path where the replica will be created. Can be either a local path such as `'my_dir.my_table'`, or a remote URI such as `'pxt://username/mydir.my_table'`.
- **`source`** (*Union[str, catalog.Table]*): Path to the source table, or (if the source table is a local table) a handle to the source table.
...
- **`base`** (*Union[catalog.Table, DataFrame]*): [`Table`](pixeltable.Table) (i.e., table or view or snapshot) or [`DataFrame`](pixeltable.DataFrame) to base the snapshot on.
...
- **`path_str`** (*str*): Path to the table.
- **`schema`** (*Optional[dict[str, Any]]*): A dictionary that maps column names to column types
- **`source`** (*Optional[TableDataSource]*): A data source from which a table schema can be inferred and data imported
- **`source_format`** (*Optional[Literal['csv', 'excel', 'parquet', 'json']]*): A hint to the format of the source data
- **`schema_overrides`** (*Optional[dict[str, Any]]*): If specified, then columns in `schema_overrides` will be given the specified types
...
- **`primary_key`** (*Optional[Union[str, list[str]]]*): An optional column name or list of column names to use as the primary key(s) of the table.
...
- **`extra_args`** (*Optional[dict[str, Any]]*): Additional arguments to pass to the source data provider
...
- **`base`** (*Union[catalog.Table, DataFrame]*): [`Table`](pixeltable.Table) (i.e., table or view or snapshot) or [`DataFrame`](pixeltable.DataFrame) to base the view on.
...
- **`table`** (*Union[str, catalog.Table]*): Fully qualified name, or handle, of the table to be dropped.
...
- *catalog.Table*: A handle to the [`Table`](pixeltable.Table).
...
- **`parameters`** (*Optional[Iterable[Union[str, exprs.ColumnRef]]]*): The columns of the table to use as parameters. If not specified, all data columns (non-computed columns) will be used as parameters.
...
```python
tools(*args: Union[func.Function, func.tools.Tool])-> func.tools.Tools
```

**Parameters:**

- **`args`** (*Union[func.Function, func.tools.Tool]*): The UDFs to use as tools.
