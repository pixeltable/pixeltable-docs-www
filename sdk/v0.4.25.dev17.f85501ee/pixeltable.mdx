...
- **`path_str`** (`str`): A name for the snapshot; can be either a simple name such as `my_snapshot`, or a pathname such as
    `dir1.my_snapshot`.
- **`base`** (`catalog.Table | DataFrame`): [`Table`](./table) (i.e., table or view or snapshot) or [`DataFrame`](./dataframe) to
...
tbl = pxt.create_table(
    'my_table', schema={'col1': pxt.Int, 'col2': pxt.String}, if_exists='ignore'
)
```
...
```python
tbl = pxt.create_table(
    'my_table', schema={'col1': pxt.Int, 'col2': pxt.Float}, if_exists='replace'
)
```
