...
## <span style={{ 'color': 'gray' }}>udf</span>&nbsp; cross_encoder_list()
...
- **`model_kwargs`** (`Json | None`): Additional keyword arguments to pass to the model, such as `strength`,
...
- **`seed`** (`Int | None`): Random seed for reproducibility.
- **`model_kwargs`** (`Json | None`): Additional keyword arguments to pass to the model, such as `num_inference_steps`,
...
tbl.add_computed_column(
    answer=question_answering(
        tbl.document_text, tbl.question, model_id='deepset/roberta-base-squad2'
    )
)
```
...
- **`normalize_embeddings`** (`Bool`): If `True`, normalizes embeddings to length 1; see the
...
```python
tbl.add_computed_column(
    result=sentence_transformer(tbl.sentence, model_id='all-mpnet-base-v2')
)
```


## <span style={{ 'color': 'gray' }}>udf</span>&nbsp; sentence_transformer_list()
...
- **`language`** (`String | None`): If using a multilingual translation model, the language code to translate to. If not provided,
...
tbl.add_computed_column(
    summary=text_summarization(
        tbl.document_text, model_id='facebook/bart-large-cnn', max_length=100
    )
)
```
...
tbl.add_computed_column(
    entities=token_classification(
        tbl.text, model_id='dbmdz/bert-large-cased-finetuned-conll03-english'
    )
)
```
