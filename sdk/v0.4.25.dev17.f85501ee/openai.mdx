...
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `chat/completions` API. For details on the available
...
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `embeddings` API. For details on the available
...
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `images/generations` API. For details on the available
...
- **`voice`** (`String`): The voice profile to use for speech synthesis. Supported options include:
    `alloy`, `echo`, `fable`, `onyx`, `nova`, and `shimmer`.
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `audio/speech` API. For details on the available
...
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `audio/transcriptions` API. For details on the available
...
```python
tbl.add_computed_column(
    transcription=transcriptions(tbl.audio, model='whisper-1', language='en')
)
```
...
- **`model_kwargs`** (`Json | None`): Additional keyword args for the OpenAI `audio/translations` API. For details on the available
...
```python
tbl.add_computed_column(
    response=vision("What's in this image?", tbl.image, model='gpt-4o-mini')
)
```
