...
```python Signature
concat_videos(videos: pxt.Json) -> Video
```
...
```python Signature
extract_frame(video: pxt.Video, *, timestamp: pxt.Float) -> Image | None
```
...
```python Signature
get_duration(video: pxt.Video) -> Float | None
```
...
```python Signature
get_metadata(video: pxt.Video) -> Json
```
