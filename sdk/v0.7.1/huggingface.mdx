...
- `DetrForSegmentationResponse`: A dictionary containing the output of the segmentation model, in the following format:
  ```python
  {
...
```python Signature
@pxt.udf
detr_to_coco(image: pxt.Image, detr_info: pxt.Json) -> CocoResponse
```
...
- `CocoResponse`: A dictionary containing the data from `detr_info`, converted to COCO format.
...
- `QuestionAnsweringResponse`: A dictionary containing the answer, confidence score, and start/end positions.
...
## <span style={{ 'color': 'gray' }}>udf</span>&nbsp; sam3_for_segmentation()
...
sam3_for_segmentation(
    image: pxt.Image,
    *,
    model_id: pxt.String = 'facebook/sam3',
    text: pxt.String | None = None,
    input_boxes: pxt.Json[(Json[(Float, ...)], ...)] | None = None,
    input_boxes_labels: pxt.Json[(Int, ...)] | None = None,
    threshold: pxt.Float = 0.5,
    mask_threshold: pxt.Float = 0.5,
    revision: pxt.String | None = None
...
- **`image`** (`pxt.Image`): The image to segment.
- **`model_id`** (`pxt.String`): The pretrained SAM 3 model to use.
- **`text`** (`pxt.String | None`): Optional concept prompt as a short noun phrase (e.g., `'yellow school bus'`).
- **`input_boxes`** (`pxt.Json[(Json[(Float`): Optional list of bounding boxes in `[x1, y1, x2, y2]` pixel coordinates that
    constrain the concept (e.g., `[[100, 150, 500, 450]]`).
- **`input_boxes_labels`** (`Any`): Optional list of integer labels for each box: `1` for positive
...
- **`threshold`** (`Any`): Confidence threshold for filtering detections.
- **`mask_threshold`** (`Any`): Threshold applied to mask logits to produce binary masks.
- **`revision`** (`Any`): The specific model revision to use (e.g., a branch, tag, or git identifier). If not
    specified, uses the default revision for the model.

**Returns:**

- `SamForSegmentationResponse`: A `SamForSegmentationResponse` containing:
...
## <span style={{ 'color': 'gray' }}>udf</span>&nbsp; sam_automatic_mask_generation()
...
sam_automatic_mask_generation(
    ...
    *,
    model_id: pxt.String = 'facebook/sam3',
    points_per_batch: pxt.Int = 64,
    points_per_crop: pxt.Int = 32,
    pred_iou_thresh: pxt.Float = 0.7,
    stability_score_thresh: pxt.Float = 0.95,
    crops_n_layers: pxt.Int = 0,
    revision: pxt.String | None = None
) -> SamForSegmentationResponse
```
...
- **`image`** (`pxt.Image`): The image to segment.
- **`model_id`** (`pxt.String`): The pretrained SAM model to use.
- **`points_per_batch`** (`pxt.Int`): Number of grid points run through the model at once. Higher values are faster but
    use more memory.
- **`points_per_crop`** (`pxt.Int`): Number of grid points sampled along each side of the image. Higher values find more
    (and smaller) objects at the cost of speed.
- **`pred_iou_thresh`** (`pxt.Float`): Quality threshold on the model's predicted mask IoU; masks scoring below it are
    discarded.
- **`stability_score_thresh`** (`pxt.Float`): Quality threshold on mask stability under mask-logit threshold changes; masks
    scoring below it are discarded.
- **`crops_n_layers`** (`pxt.Int`): Number of additional image-crop layers to run. Values above `0` improve recall on
    small objects at the cost of speed.
- **`revision`** (`pxt.String | None`): The specific model revision to use (e.g., a branch, tag, or git identifier). If not
    specified, uses the default revision for the model.
...
text_classification(
    ...
    *,
    model_id: pxt.String,
    top_k: pxt.Int = 5
) -> pxt.Json[(Json[{'label': Int, 'label_text': String, 'score': Float}], ...)]
```
...
- `pxt.Json[(Json[{'label': Int, 'label_text': String, 'score': Float}], ...)]`: A dictionary containing classification results with scores, labels, and label text.
...
) -> pxt.Json[(Json[{'word': String, 'entity_group': String, 'score': Float, 'start': Int, 'end': Int}], ...)]
```
...
- `pxt.Json[(Json[{'word': String, 'entity_group': String, 'score': Float, 'start': Int, 'end': Int}], ...)]`: A list of dictionaries containing entity information (text, label, confidence, start, end).
...
vit_for_image_classification(
    ...
    *,
    model_id: pxt.String,
    top_k: pxt.Int = 5
) -> VitForImageClassificationResponse
```
...
- `VitForImageClassificationResponse`: A dictionary containing the output of the image classification model, in the following format:
  ```python
  {
