...
- **`im2`** (`Image`): The second image.  Must have mode RGBA, and the same size as
  the first image.
...
- **`im2`** (`Image`): The second image.  Must have the same mode and size as
  the first image.
- **`alpha`** (`Float`): The interpolation alpha factor.  If alpha is 0.0, a
...
- **`image2`** (`Image`): The second image.  Must have the same mode and
  size as the first image.
- **`mask`** (`Image`): A mask image.  This image can have mode
...
- **`alpha_only`** (`Bool`): Optional flag, defaulting to ``True``.
...
- `Json`: The bounding box is returned as a 4-tuple defining the
...
- **`channel`** (`Int`): What channel to return. Could be index
  (0 for "R" channel of "RGB") or channel name
  ("A" for alpha channel of "RGBA").

**Returns:**

- `Image`: An image in "L" mode.
  .. versionadded:: 4.3.0
...
- **`maxcolors`** (`Int`): Maximum number of colors.  If this number is
...
- `Json`: For a single-band image, a 2-tuple containing the
...
- **`rawmode`** (`Any`): The mode in which to return the palette. ``None`` will
  return the palette in its current mode.
  .. versionadded:: 9.1.0

**Returns:**

- `Json`: A list of color values [r, g, b, ...], or None if the
...
- `Json`: Two sequences, indicating where there are non-zero
...
- **`method`** (`Int | None`): `Quantize.MEDIANCUT` (median cut),
...
- **`palette`** (`Int | None`): Quantize to the palette of given
  `PIL.Image.Image`.
- **`dither`** (`Int`): Dithering method, used when converting from
...
- **`factor`** (`Int`): A greater than 0 integer or tuple of two integers
  for width and height separately.
- **`box`** (`Json | None`): An optional 4-tuple of ints providing
...
- **`method`** (`Int`): One of :py`Transpose.FLIP_LEFT_RIGHT`,
  :py`Transpose.FLIP_TOP_BOTTOM`, :py`Transpose.ROTATE_90`,
  :py`Transpose.ROTATE_180`, :py`Transpose.ROTATE_270`,
  :py`Transpose.TRANSPOSE` or :py`Transpose.TRANSVERSE`.
