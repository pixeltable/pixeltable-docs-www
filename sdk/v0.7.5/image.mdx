...
## <span style={{ 'color': 'gray' }}>uda</span>&nbsp; stitch_tiles()

```python Signature
@pxt.uda
stitch_tiles(*args, **kwargs) -> pxt.Image | None
```

Reconstructs a single image from the tiles produced by [`tile_iterator`](./image#iterator-tile_iterator),
pasting each tile back at its original position. This is the inverse of
[`tile_iterator`](./image#iterator-tile_iterator):  aggregate the tiles of an image (grouped by a column
...
Args:
    ...
    [`tile_iterator`](./image#iterator-tile_iterator). All tiles in a group must have the same mode
...
Examples:
    ...
    [`tile_iterator`](./image#iterator-tile_iterator), with an `overlay` column computed per tile:
...
```python Signature
@pxt.udf
get_metadata(self: pxt.Image) -> ImageMetadata
```
