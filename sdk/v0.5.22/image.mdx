...
- **`tile_size`** (`pxt.Json`): Size of each tile, as a pair of integers `(width, height)`.
- **`overlap`** (`pxt.Json`): Amount of overlap between adjacent tiles, as a pair of integers `(width, height)`.
